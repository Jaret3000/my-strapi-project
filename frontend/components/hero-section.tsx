...
const styles = {
    ...
    heading: "text-4xl font-bold md:text-xl lg:text-2xl",
...
export function HeroSection({data}: {readonly data: {
    ...
    const imageURL = data.image?.url.startsWith('http') ? data.image.url 
    : `${STRAPI_BASE_URL}${data.image.url}`;

    return (
        <header className={styles.header}>
            <img alt="Background" className={styles.backgroundImage} height={1080} 
            src={imageURL}
            ...
            <div className={styles.overlay}>
                ...
                <Link className={styles.button} href={link.href}>{link.label}</Link>
            </div>
        </header>
    );
}